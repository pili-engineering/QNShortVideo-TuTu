...
// TuSDK mark - 导入
#import <TuSDKVideo/TuSDKVideo.h> // 音频处理
...
- (CVPixelBufferRef)shortVideoRecorder:(PLShortVideoRecorder *)recorder cameraSourceDidGetPixelBuffer:(CVPixelBufferRef)pixelBuffer {
    ...
    // 禁用七牛短视频 SDK 内部自带滤镜，充分展示 涂图滤镜、美颜效果
//    if (self.isUseFilterWhenRecording) {
...
//    }
    ...
    if ([TuSDKManager sharedManager].isInitFilterProgress == NO) {
        return pixelBuffer;
    }
    pixelBuffer =  [[TuSDKManager sharedManager] syncProcessPixelBuffer:pixelBuffer];
    return pixelBuffer;
}
...
- (void)initTUSDK {
    ...
    // TuSDK mark
    [[TuSDKManager sharedManager] setIsShowDistortingMirror:YES];
    [[TuSDKManager sharedManager] configFilterCodes:@[kCameraFilterCodes]];
    [[TuSDKManager sharedManager] configCartoonCodes:@[kCameraCartoonCodes]];
    [[TuSDKManager sharedManager] configBeautySkinKeys:@[kBeautySkinKeys]];
    [[TuSDKManager sharedManager] configBeautyFaceKeys:@[kBeautyFaceKeys]];
    
    [[TuSDKManager sharedManager] configSuperView:self.view outputType:TuSDKManagerFilterOutputType_32BGRA];
