...
- (void)commonInit {
    ...
    _normalFilterListView = [[CameraNormalFilterListView alloc] initWithFrame:CGRectZero filterCodes:filterCodes];
    ...
    if ([TuSDKManager sharedManager].isShowCartoonView) {
        // 漫画滤镜列表
    ...
    if ([TuSDKManager sharedManager].isShowCartoonView) {
        tabbar.itemTitles = @[NSLocalizedStringFromTable(@"tu_滤镜", @"TuSDKConstants", @"滤镜"), NSLocalizedStringFromTable(@"tu_漫画", @"TuSDKConstants", @"漫画")];
    } else {
        tabbar.itemTitles = @[NSLocalizedStringFromTable(@"tu_滤镜", @"TuSDKConstants", @"滤镜")];
    }
...
- (NSInteger)numberOfViewsInSlider:(ViewSlider *)slider {
    return [TuSDKManager sharedManager].isShowCartoonView ? 2 : 1;
}
