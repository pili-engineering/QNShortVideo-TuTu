...
- (void)commonInit {
    ...
    NSArray *faceFeatures = [TuSDKManager sharedManager].beautySkinKeys;
    _useSkinNatural = YES;
    // 配置 UI
    ...
    [self addItemViewsWithCount:faceFeatures.count config:^(HorizontalListView *listView, NSUInteger index, HorizontalListItemView *_itemView) {
        ...
        UIImage *image = [[UIImage imageNamedWithNormalBundle:imageName] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        _itemView.thumbnailView.image = image;
    }];
    
    // 重置按钮
    HorizontalListItemView *resetItemView = [CameraBeautyFaceListItemView itemViewWithImage:[UIImage imageNamedWithNormalBundle:@"ic_nix"] title:nil];
...
{
    if (self.selectedIndex > 2 && self.selectedIndex - 2 < [TuSDKManager sharedManager].beautySkinKeys.count)
        return [TuSDKManager sharedManager].beautySkinKeys[self.selectedIndex - 2];
    return nil;
}
